...
const PricingCard= ({name,price,description,items,id,paymentLink}:PriceType) => {
    return (
        <div className={"relative w-full max-w-lg hover:scale-105 hover:transition-all duration-300"}>
           <div className={cn("relative flex flex-col h-full gap-4 lg:gap-8 z-10 p-8 border-[1px] border-gray-500/20 rounded-2xl",id === 'pro' && "border-[#E9A5F1] gap-5 border-2")}>
            ...
            <div className={"space-y-2 justify-center w-full"}>
                <Link href={paymentLink}   className={cn(
                    ...
                    "    bg-[length:200%_200%] hover:bg-[position:100%_100%]", id === 'pro' ? 'border-purple-500': 'border-rose-100 ')
                }
                >
                    {id === "pro" ? (
